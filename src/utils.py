...
def print_memory_summary(prefix: str = "") -> None:
    ...
    for device, _mem in stats.items():
...
def move_model_to_device(model: torch.nn.Module, device: str | torch.device) -> torch.nn.Module:
...
def print_model_summary(model: torch.nn.Module, input_shape: tuple | None = None) -> None:
    """Print comprehensive model summary."""

    stats = get_parameter_stats(model)

    # Memory estimate
    estimate_model_memory(stats["total"])
...
class Timer:
    ...
    def __init__(self, name: str = "Operation", logger: Callable | None = None):
        self.name = name
...
class ProgressTracker:
    ...
    def _print_progress(self) -> None:
        ...
        (self.total - self.current) / rate if rate > 0 else 0
...
def create_experiment_dir(base_dir: str | Path, experiment_name: str) -> Path:
...
def backup_file(path: str | Path, backup_dir: str | Path | None = None) -> Path:
